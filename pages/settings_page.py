...
def _open_reg_key(open_mode):
    try:
        import winreg
        hkey = winreg.OpenKey(winreg.HKEY_CURRENT_USER, _REG_KEY, 0, winreg.KEY_READ | open_mode)
        return hkey, winreg
    except ImportError:
        return None, None
...
def get_settings_path():
    data_dir = os.path.join(os.path.expanduser("~"), "BOM管理系统数据")
...
class SettingsPage(ctk.CTkFrame):
    ...
    def _build_data_management(self):
        ...
        if not current_dir:
            current_dir = os.path.join(os.path.expanduser("~"), "BOM管理系统数据")
    ...
    def _build_startup(self):
        ...
        ctk.CTkLabel(card,
                     text="勾选后，BOM管理系统将在 Windows 启动时自动运行",
                     font=ctk.CTkFont(family="Microsoft YaHei", size=11),
    ...
    def _build_version_info(self):
        ...
        ctk.CTkLabel(
            card, text="轻量、专注的产品BOM管理桌面工具，覆盖成品信息管理、BOM物料清单查询等全流程。",
            font=ctk.CTkFont(family="Microsoft YaHei", size=12),
            text_color=self.C["text_secondary"],
        ...
        modules = [
            ("📋 成品信息", "管理成品基本信息，支持按渠道、项目号、品名筛选，支持Excel导入/导出"),
            ("🏗️ 成品BOM", "管理成品物料清单（BOM），支持多条件模糊检索，查看成品与物料对应关系"),
    ...
    def _build_about_author(self):
        ...
        if os.path.exists(wx_qr_path) and PIL_AVAILABLE:
            try:
                ...
                wx_label = ctk.CTkLabel(wx_frame, image=wx_img, text="")
                wx_label.image = wx_img
        ...
        repo_btn = ctk.CTkButton(
            github_frame, text="eastseao / bom-management",
            width=280, height=34,
            fg_color=self.C["primary"], hover_color=self.C["primary_hover"],
            font=ctk.CTkFont(family="Microsoft YaHei", size=12, weight="bold"),
            corner_radius=6,
    ...
    def _open_data_dir(self):
        ...
        if not path:
            path = os.path.join(os.path.expanduser("~"), "BOM管理系统数据")
    ...
    def _save_all(self):
        ...
        if old_data_dir != new_data_dir:
            default_dir = os.path.join(os.path.expanduser("~"), "BOM管理系统数据")
            src_dir = old_data_dir if old_data_dir else default_dir
            dst_dir = new_data_dir if new_data_dir else default_dir
            src_db = os.path.join(src_dir, "bom_system.db")
            dst_db = os.path.join(dst_dir, "bom_system.db")
